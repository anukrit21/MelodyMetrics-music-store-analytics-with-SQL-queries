...
-- Q16. Customer Retention Rate

WITH CustomerPurchases AS (
    SELECT customer_id, COUNT(DISTINCT invoice_id) AS PurchaseCount
    FROM invoice
    GROUP BY customer_id
)
SELECT
    ROUND(
        (SUM(CASE WHEN PurchaseCount > 1 THEN 1 ELSE 0 END) / COUNT(*)) * 100,
        2
    ) AS RetentionRate
FROM CustomerPurchases;

-- Q17. Cross-Genre Recommendation System
...
SELECT
    cg.customer_id,
    g.name AS RecommendedGenre
FROM CustomerGenres cg
JOIN genre g ON g.genre_id NOT IN (
    SELECT genre_id
    FROM track t
    JOIN genre g ON t.genre_id = g.genre_id
    WHERE t.track_id IN (
        ...
        WHERE i.customer_id = cg.customer_id
    )
)
GROUP BY cg.customer_id, g.name;
